...
<section class="content"  style="direction:rtl">
      <div class="row">
        ...
        <div class="panel-heading">فاتورة مردود مبيع</div>
        <div>
        @foreach($catch6 as $catchs6)
         <a href='/adminpanel/inv/{{$catchs6->id}}' target="_blank"><span class="glyphicon glyphicon-print"></span></a> 
          @endforeach
  ...
  <form  name="ordersave" action="{{action('InvoiceController@resave')}}" onsubmit="return validateForm()" method="post">
  <input type="hidden" name="_token" value="<?= csrf_token() ; ?>" >

  
  <table class="table" style="direction:rtl">
                            <tr>
                             <input type="hidden" name="orderType" value="resell">
                            </tr>
                            <tr>
                             <th><span>رقم المعرف</span></th>
                                             <td>  
                                             @foreach($catch6 as $catchs6)
                                                {{($catchs6->id)+1}}
                                            @endforeach
                       ...
                       <td> 
                         ...
                         <select name="orderPaymentType" id="orderPaymentType" class="form-control" required oninvalid="this.setCustomValidity('من فضلك حدد نوع الدفع')" oninput="setCustomValidity('')">
                          ...
                          <option value="cash">الصندوق</option>
                            ...
                            <td><span><input type="text" name="getMoney" class="getMoney form-control" required oninvalid="this.setCustomValidity('من فضلك أدخل المبلغ المدفوع')" oninput="setCustomValidity('')" ></span></td> 
                            <th><span>الباقي:</span></th>
                            <td><span><input type="text"  name="backMoney" class="backMoney form-control" readonly></span></td>
      ...
      <tr>
        ...
        <option selected="selected" > اختر مادة</option>
          <?php foreach ($data as $r) {
            ...
            <option data-price="<?= $r->productSellPrice; ?>" value="<?= $r->id; ?>"> <?= $r->productName ?></option>
            <?php 
          }
          ?>
        </select>
