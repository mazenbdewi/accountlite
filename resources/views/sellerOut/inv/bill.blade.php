...
<html dir="rtl" lang="ar">
    <head>
        <meta charset="utf-8">
        <title>
        <?php  if($order->orderType=='sellOut') echo 'فاتورة بيع' ;
                                      elseif($order->orderType=='buy') echo 'أمر شراء';
...
function printDiv(divName) {
    ...
    <body>
    <a href="#!" class="btn-floating orang" onclick="printDiv('printableArea')" value="Print" style="float:right;"><span class="glyphicon glyphicon-print">طباعة</span></a>
    &nbsp;  &nbsp;  &nbsp;  &nbsp;
 
  <div class="below_movie_left" id="printableArea">
        <header>
            <h1> <?php  if($order->orderType=='sellOut') echo 'فاتورة بيع' ;
                                      elseif($order->orderType=='buy') echo 'أمر شراء';
                                      elseif($order->orderType=='rebuy') echo 'فاتورة مرتجع مشتريات';
                                      elseif($order->orderType=='resell') echo 'فاتورة مرتج مبيعات';
                                      elseif($order->orderType=='sandout') echo 'سند صرف';
        ...
        <article>
            ...
            <table >
               ...
               <?php  if($order->orderType=='sellOut'){
                ...
                elseif($order->orderType=='buy'){
                     echo '<th><span >أمر شراء رقم</span></th>';
    ...
    @if($order->orderType == 'sandin')
        ...
        <td><span >{{number_format($order->getMoney,2)}}</span><span data-prefix>ريال</span></td> 
        </tr>
        <table>
    ...
    @else
        ...
        @if($order->orderType =='sellOut' || $order->orderType =='buy'  )
            <table class="inventory">
                  ...
                  @else
                   @foreach($productorders as $index => $productorder)
                    ...
                    <td><span > {{ $productorder->sellDisc }}</span><span data-prefix>%</span></td>
                         ...
                         <td><span>{{number_format($productorder->sellAllprice,2)}}</span><span data-prefix>ريال</span></td>
                    </tr>
                      @endforeach  
                     <table class="balance">
                <tr>
                <?php $totalsorder=0 ; ?>
                 ...
                 @endforeach
                    <th><span >المجموع العام</span></th>
                    <td><span>{{number_format($totalsorder,2)}}</span><span data-prefix>ريال</span></td>
